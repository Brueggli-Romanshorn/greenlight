class AddSettingAdministrativeTexts < ActiveRecord::Migration[7.2]
  def up
    ...
    SiteSetting.create(setting: Setting.find_by(name: 'PrivacyText'), value: '[{"type":"paragraph","children":[{"text":"Privacy Policy"}]}]', provider: 'greenlight') unless SiteSetting.exists?(setting: Setting.find_by(name: 'PrivacyText'), provider: 'greenlight')
